...
public class TrainingCourses{
	...
	public void removeEmployeesFromCourse() {
		...
		do {
			...
			removedCount = 0;  //counter to return number of employees removed from that course
			found = false;  //to see if any employees were removed
			...
			while (current != null) {
				if (current.getElement().getCourseName().equalsIgnoreCase(courseRemove)) { //if match found, remove
					...
					// once removed, reset current and previous to continue correctly
					if (previous == list.getFront()) {
						current = list.getFront();  //reset to front of list
						previous = list.getFront();  //reset to front of list
					} else {
